...
import ListFilter from './soho-listfilter';
...
export default class ListView {
   ...
   */
  getTotals(dataset: any) {
    const totals: any = { count: dataset.length };
    let property;
   ...
   */
  render(dataset: any, pagerInfo?: any) {
    const self = this;
    ...
    items.each(function (i: any, elem: any) {
      const item: any = $(elem);
    ...
    // Handle refresh
    this.element.off('updated.listview').on('updated.listview', (e: any, settings: any) => {
   ...
   * @returns {void}
   */
  renderPager(updatedPagerInfo: any, isResponse: any) {
    if (!this.pagerAPI) {
      return;
    }
   ...
   */
  refresh(dataset?: any, pagingInfo?: any) {
   ...
   * @returns {void}
   */
  loadData(ds?: Record<string, unknown> | null, pagerInfo?: any, isResponse?: boolean) {
    let ajaxDs = false;
    const self = this;
    ...
    if (!pagerInfo) {
      if (this.pagerAPI) {
        if (ds && ds.length !== this.pagerAPI.settings.dataset.length) {
    ...
    if (this.filteredDataset) {
      pagerInfo.filteredTotal = ds ? ds.length : 0;
    } else {
      pagerInfo.total = ds ? ds.length : 0;
    }
    ...
    function done(response: any, pagingInfo: any) {
    ...
    if (typeof ds === 'string' && ((ds as any).indexOf('http') === 0 || (ds as any).indexOf('/') === 0)) {
      s = ds;
      ajaxDs = true;
    }
   ...
   * @returns {void}
   */
  selectItemsBetweenIndexes(indexes: any) {
    utils.clearSelection(); // Clear all currently selected list items.
    indexes.sort((a: any, b: any) => a - b);
  ...
  * @returns {void}
  */
  deselectItemsBetweenIndexes(indexes: any) {
    indexes.sort((a: any, b: any) => a - b);
    for (let i = indexes[0]; i <= indexes[1]; i++) {
      const item = $('li, tbody tr', this.element).eq(i);
  ...
  */
   ...
   * @returns {void}
   */
  handleSearch(e: any, searchfield: any) {
  ...
  filter(searchfield: any) {
    ...
    // Reset filter status
    this.settings.dataset.forEach((item: any) => {
  ...
  resetSearch() {
    ...
    this.settings.dataset.forEach((item: any) => {
      ...
      if (this.settings.hasChildren) {
        item[this.settings.children].forEach((child: any) => {
   ...
   * @returns {void}
   */
  sortInit(control: any, onEvent: any, attr: any) {
    if (!attr || $.trim(attr) === '') {
      return;
    }
    $(`[${attr}]`).each(function () {
      const element = $(this);
      const options = ($.fn as any).parseOptions(element, attr);

      element.on(onEvent, (e) => {
        ($(options.list).data(control) as any).setSortColumn(options);
        e.preventDefault();
      });
    });
  }
  ...
  */
  sortFunction(field: any, reverse: any, primer?: any) {
    if (!primer) {
      primer = function (a: any) {
    ...
    const key = primer ? function (x: any) {
    ...
    return function (a: any, b: any) {
      return a = key(a), b = key(b), reverse * (((a as any) > (b as any) as any) - ((b as any) > (a as any) as any));
    };
  }
  ...
  */
   ...
   */
  select(li: any, noTrigger: any = false, isKey = false) {
    const self: any = this;
    let isChecked = false;
    ...
    li.parent().find('.is-selected').each(function (i: any, elem: any) {
      self.selectedItems[i] = $(elem);
    });
    ...
    if (!noTrigger) {
      ...
      for (let i = 0; i < this.selectedItems.length; i++) {
        if (!this.selectedItems[i][0].getAttribute) {
          selectedData.push(1);
          return;
        }
    ...
    if (self.selectedItems.length > 0) {
      ...
      toolbar.addClass('is-hidden').one('animateclosedcomplete', function (e: any) {
        e.stopPropagation();
  ...
  */
  ...
  handleEvents() {
    ...
    this.element
      ...
      .on('focus.listview', 'li, tbody tr', function (evt: any, elem: any) {
        const item = $(elem);
    ...
    this.element
      ...
      .on('keydown.listview', 'li, tr, a', function (e: any) {
        const li: any = $(e.currentTarget);
        const item = li.is('a') ? li.closest('li') : li;
        ...
        if ((key === 40 || key === 38) && !metaKey) { // move down or up
          ...
          if (newItem.length && ($(e.target).is(item) || e.shiftKey || li.is('a'))) {
            self.focus(newItem);
          }
          e.preventDefault();
    ...
    if (this.settings.selectable) {
      ...
      this.element
        ...
        .on('click.listview', 'li, tr, input[checkbox]', function (e: any) {
          ...
          if (pattern.length > 0 && ($(window) as any).outerWidth() < 767 && !item.hasClass('is-disabled') && !isCheckbox) {
          ...
          self.element.trigger('click', [{
            elem: item,
            data: self.settings.dataset[(item as any).attr('aria-posinset') - 1],
            index: item.index(),
            originalEvent: e
          }]);
        });
      ...
      this.element
        ...
        .on('dblclick.listview', 'li, tr', function (e: any) {
          const item = $(e.currentTarget);
      ...
      this.element
        ...
        .on('contextmenu.listview longpress.listview', 'li, tr', function (e: any) {
          const item = $(e.currentTarget);
    ...
    if (this.settings.selectable === 'multiple' || this.settings.selectable === 'mixed') {
      this.element
        ...
        .on('change.selectable-listview', '.listview-checkbox input', function (e: any) {
    ...
    if (this.settings.searchable) {
      this.searchfield
        ...
        .on('contents-checked.searchable-listview', function (e: any) {
          self.handleSearch(e, $(e.currentTarget));
        })
    ...
    if (this.pagerAPI) {
      this.element
        .off('page.listview').on('page.listview', (e: any, pagingInfo: any) => {
          self.handlePageChange(pagingInfo);
        })
        .off('pagesizechange.listview').on('pagesizechange.listview', (e: any, pagingInfo: any) => {
    ...
    self.element.prev('.listview-header')
      .off('click.listview')
      .on('click.listview', function (e: any) {
        const icon = $(e.currentTarget).find('.plus-minus');
   ...
   */
  handlePageSizeChange(pagingOpts: any) {
   ...
   * @param {object} pagingOpts state information from the pager
   */
  handlePageChange(pagingOpts: any) {
    this.loadData(undefined, pagingOpts);
  }
}
