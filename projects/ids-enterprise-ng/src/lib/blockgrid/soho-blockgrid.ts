...
function handleLegacySettings(storedSettings: any, incomingSettings: any) {
  if (!incomingSettings) {
...
export default class Blockgrid {
  ...
  handleEvents() {
    ...
    this.element.on(`click.${COMPONENT_NAME}`, '.soho-block', (e: any) => {
    ...
    this.element.on(`focus.${COMPONENT_NAME}`, '.checkbox', (e: any) => {
      const block = $(e.currentTarget).parent();
      block.addClass('has-focus');
    });

    this.element.on(`blur.${COMPONENT_NAME}`, '.checkbox', (e: any) => {
      const block = $(e.currentTarget).parent();
      block.removeClass('has-focus');
    });

    this.element.on(`keypress.${COMPONENT_NAME}`, '.soho-block', (e: any) => {
      if (e.which !== 32) {
        return;
      }
   ...
   * @returns {void}
   */
  selectBlock(activeBlock: any, isCheckbox: boolean) {
   ...
   * @param {boolean} isCheckbox True if a checkbox, used for mixed mode.
   */
  select(activeBlock: any, isCheckbox: boolean) {
    const allBlocks = this.element.find('.soho-block');
  ...
  render() {
    ...
    for (let i = 0; i < displayedDataset.length; i++) {
      ...
      if (s.withImage) {
        ...
        blockElement = `<div class="soho-block is-selectable${selected}" role="listitem" tabindex="0">
        ...
        blockElement = `<div class="soho-block text-block is-selectable${selected}" role="listitem" tabindex="0">
        <input type="checkbox" aria-hidden="true" role="presentation" class="checkbox" id="checkbox${i}" tabindex="${tabindex}" data-idx="${data.id || i}"${checked}>
    ...
    if (this.previousFocusedElement) {
      setTimeout(() => {
        this.element.find('.soho-block').first().focus();
