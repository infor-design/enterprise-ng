...
export default class TagList {
  ...
  render() {
    ...
    const elemClassArr = utils.getArrayFromList(this.element.classList as any);
    const currentState = this.settings.style;

    // Update "style" class on the top-level element
    elemClassArr.forEach((cssClass: string) => {
  ...
  getSettingsFromElement() {
    ...
    // Text Content
    const contentElem: any = this.contentElement;
    if (contentElem) {
  ...
  remove() {
    ...
    let parentNode: any = thisNode.parentNode;
  ...
  teardown() {
    ...
    const dismissibleBtn: any = this.dismissibleBtn;
    if (dismissibleBtn) {
