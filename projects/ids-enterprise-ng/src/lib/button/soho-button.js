...
const pressableTypes = ['soho-icon-favorite', 'soho-btn-toggle'];
...
Button.prototype = {
  ...
  render() {
    ...
    if (buttonsetAPI) {
      ...
      // btn
      const btnClasses = ['default', 'soho-btn', 'soho-soho-btn-modal'];
      if (btnClasses.indexOf(this.settings.style) > -1) {
        elemClasses[!switchModalClasses ? 'add' : 'remove']('soho-btn');
        elemClasses[switchModalClasses ? 'add' : 'remove']('soho-btn-modal');
      }

      // soho-btn-primary
      const btnPrimaryClasses = ['soho-btn-primary', 'soho-btn-modal-primary'];
      if (btnPrimaryClasses.indexOf(this.settings.style) > -1) {
        elemClasses[!switchModalClasses ? 'add' : 'remove']('soho-btn-primary');
        elemClasses[switchModalClasses ? 'add' : 'remove']('soho-btn-modal-primary');
      }

      // btn-secondary
      const btnSecondaryClasses = ['soho-btn-secondary', 'soho-btn-modal-secondary'];
      if (btnSecondaryClasses.indexOf(this.settings.style) > -1) {
        elemClasses[!switchModalClasses ? 'add' : 'remove']('soho-btn-secondary');
        elemClasses[switchModalClasses ? 'add' : 'remove']('soho-btn-modal-secondary');
      }
    }

    // Backwards-compatibility with a legacy Modal setting.
    // `isDefault` is equivalent to setting a `soho-btn-modal-primary` class.
    if (this.settings.isDefault) {
      this.settings.style = 'soho-btn-primary';
      elemClasses.remove('soho-btn-primary');
      elemClasses.add('soho-btn-modal-primary');
    ...
    if ((this.element.hasClass('soho-btn-tertiary') && this.element.hasClass('soho-btn-generative')) ||
      this.element.attr('soho-button') === 'tertiary-generative') {

      this.element.find('span + svg').remove();
    ...
    const audibleTextBtnTypes = ['soho-btn-icon', 'soho-btn-actions'];
    ...
    } else if (this.settings.type === 'soho-btn-toggle') {
      if (this.pressed) {
    ...
    } else if (this.settings.type === 'soho-btn-actions') {
    ...
    if (this.element.hasClass('soho-btn-menu') && !this.element.hasClass('soho-btn-icon') && !this.element.hasClass('soho-btn-actions')) {
    ...
    if (this.element.hasClass('soho-btn-actions')) {
  ...
  getSettingsFromElement(dontStoreSettings = false) {
    ...
    if (this.buttonsetAPI && this.buttonsetAPI.settings.style === 'modal') {
      const modalBtnMappings = {
        'soho-btn-modal': 'soho-btn',
        'soho-btn-modal-primary': 'soho-btn-primary',
        'soho-btn-modal-secondary': 'soho-btn-secondary'
      };
  ...
  removeGenerativeStyles() {
    ...
    if ($elem.hasClass('soho-btn-tertiary') && $elem.hasClass('soho-btn-generative')) {
  ...
  teardown() {
    ...
    if (this.element.hasClass('soho-btn-actions') && tooltipApi) {
      tooltipApi.destroy();
    }
